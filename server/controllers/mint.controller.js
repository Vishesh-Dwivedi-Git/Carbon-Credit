...
// Create a new incentive (Government action)
export async function createIncentive(req, res, next) {
  try {
    ...
    // Mint tokens for the incentive pool (e.g., for future distribution)
    const tokenAmount = ethers.parseUnits((points * 10).toString(), 18); // 10 CCt per point, assuming 18 decimals
    if (tokenAmount > 0) {
      const treasuryAddress = process.env.TREASURY_ADDRESS; // Add to .env
      const tx = await tokenContract.mint(treasuryAddress, tokenAmount);
      await tx.wait();
    }
...
// Get all incentives (No changes needed)
...
// Update an incentive (No blockchain interaction needed)
...
// Delete an incentive (No blockchain interaction needed)
...
export async function awardIncentivePoints(req, res, next) {
  try {
    ...
    // Transfer CCt tokens to the organization's wallet
    if (incentive.category === "carbon_offset" && carbonOffset > 0) {
      try {
        const tokenAmount = ethers.parseUnits(
          Math.floor(carbonOffset / 10).toString(),
          18
        ); // 1 CCt per 10 units of carbon offset
        if (tokenAmount > 0) {
          const tx = await tokenContract.TransferFromOwnerOnLogin(
            org.walletAddress,
            tokenAmount
          );
          await tx.wait();
          org.CCtTokens += Number(ethers.formatUnits(tokenAmount, 18));
          await org.save();
        }
      } catch (err) {
        console.error("Token transfer failed:", err);
        // Continue execution even if token transfer fails
      }
    }
...
// Get user incentives (No blockchain interaction needed)
export async function getUserIncentives(req, res, next) {
  try {
    const userId = req.params.userId || req.user.id;

    const userIncentives = await UserIncentive.find({ user: userId })
      .populate("incentive")
      .sort({ createdAt: -1 });

    const totalPoints = userIncentives.reduce(
      (sum, record) => sum + record.pointsEarned,
      0
    );
    const totalCarbonOffset = userIncentives.reduce(
      (sum, record) => sum + record.carbonOffset,
      0
    );
...
// Get leaderboard (No blockchain interaction needed)
...
export async function redeemIncentivePoints(req, res, next) {
  try {
    ...
    switch (rewardType) {
      case "token_bonus":
        const tokenBonus = ethers.parseUnits(
          Math.floor(pointsUsed / 50).toString(),
          18
        ); // 1 CCt per 50 points
        try {
          const tx = await tokenContract.transfer(org.walletAddress, tokenBonus);
          await tx.wait();
          org.CCtTokens += Number(ethers.formatUnits(tokenBonus, 18));
          await org.save();
          reward = { type: "token_bonus", amount: Number(ethers.formatUnits(tokenBonus, 18)) };
        } catch (err) {
          console.error("Token transfer failed:", err);
...
// Get global impact report (No blockchain interaction needed)
